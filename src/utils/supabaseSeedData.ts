...
export const sampleMemoData: MemoFormData[] = [
  {
    title: '프로젝트 회의 준비',
    content:
...
export const seedSupabaseData = async (): Promise<boolean> => {
  try {
    // 기존 데이터가 있는지 확인
    ...
    if (existingMemos.length === 0) {
      ...
      // 샘플 데이터를 하나씩 추가
      for (const memoData of sampleMemoData) {
        const result = await supabaseUtils.addMemo(memoData)
        if (!result) {
          console.error('Failed to seed memo:', memoData.title)
          return false
        }
      }
      
      console.log('Sample data seeded successfully to Supabase!')
      return true
    }
...
export const clearSupabaseData = async (): Promise<boolean> => {
  try {
...
export const resetSupabaseToSampleData = async (): Promise<boolean> => {
  try {
    // 기존 데이터 삭제
